...
def imshow1(img, Name):
    ...
    plt.imshow(img)
    plt.axis('off')
    plt.savefig(os.path.join('/home/.../out/', Name+'.png'), bbox_inches='tight', pad_inches=0)
...
class rootLoader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        #csv_path = self.root + "/" + self.split + "csv/" + img_name[:-4]+'.csv'
        #pri_csv_path = self.root + "/" + self.split + "csvpri/" + img_name[:-4]+'.csv'
        #lat_csv_path = self.root + "/" + self.split + "csvsec/" + img_name[:-4]+'.csv'
        TRSML = self.root + "/" + self.split + "RSML/" + img_name[:-4]+'.rsml'
        #VRSML = self.root + "/" + self.split + "annotRSML/" + img_name[:-4]+'.rsml'
        ...
        h, w = float(h), float(w)
        #a = ((new_h / h)+0.006, (new_w / w)+0.0001)      
        a = ((new_h / h), (new_w / w))      
...
if __name__ == "__main__":
    ...
    dst = rootLoader(local_path, is_transform=True, augmentations=augmentations)
    bs = 4
