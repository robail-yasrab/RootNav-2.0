...
def _normalise(a):
    pad = a.shape[1] // 25
    a -= a[:,pad:-pad,pad:-pad].min()
    a /= a[:,pad:-pad,pad:-pad].max()
...
class Hourglass(nn.Module):
    ...
    def _hour_glass_forward(self, n, x):
        ...
        for i in range(0, 127):
            ...
            a = Image.fromarray(np.squeeze(a)*255 ).convert("RGB")
            a.save('./maps/'+str(i)+'HG-EN-'+str(q)+'.png')
        ...
        for i in range(0, 127):
            ...
            a = a.detach().cpu().numpy()
            a = Image.fromarray(np.squeeze(a)*255 ).convert("RGB")
            a.save('./maps/'+str(i)+'HG-DE-'+str(q)+'.png')        
        out = up1 + up2
