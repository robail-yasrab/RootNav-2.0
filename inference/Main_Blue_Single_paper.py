...
def test():
	for extension in fileExtensions:
	    ...
	    for x in range(0, i):
	        ...
	        #img2= ext_color(decoded_crf, my_list11, my_list12)
	        ...
	        for idxx, i in enumerate(goal):
	            my_list4.append([])
	            my_list5.append([])
	            path = AStar_Lat(i, Lat_goal, von_neumann_neighbors, distance, heuristic, img3)
	            if path !=[]:
	                ...
	                for s,t in zip(my_list4, my_list5):
	                    ...
	                    for p in neighbors:
	                        ...
	                        if int(p[0]) in s and int(p[1]) in t and id_lat not in count_lat and p[0]!=[] and p[1]!=[]:
	                            ...
	                            polyline = ET.SubElement(geometry, 'polyline')
	                            rootnavspline = ET.SubElement(geometry, 'rootnavspline', controlpointseparation= "80", tension="0.5")
	                            pointss = [] 
	                            for n, p in zip(s, t):
	                                pointss.append((n, p))
	                                n = np.array(n)
	                            ...
	                            ss = Spline(pointss, tension = 0.5, knot_spacing = 80)
	        ...
	        plt.figure()
	        plt.imshow(img007)
	        plt.scatter(my_list8, my_list9, s=2, marker='.', c='b')
...
if __name__ == '__main__':
	# Setup Model
	model_path = "./models/osr_bluepaper.pkl"
	#model_path = "./Model/blue.pkl"
	model = hg()
